...
export async function askClaude(userText) {
  ...
  const options = {
    ...
    // 排除系统提示词中的动态部分（cwd、git status、memory），使其完全静态，
    // 每次 query() 调用生成相同的系统提示词，最大化 DeepSeek prompt 缓存命中率。
    // 动态内容会被注入到第一条 user message 中，模型仍可访问。
    // 注：resume 时此选项被 SDK 忽略（复用已有会话的系统提示词），无副作用。
    systemPrompt: {
      type: "preset",
      preset: "claude_code",
